      ...
      {quantity === 0 ? (
          ...
          <>
            <div className="d-flex align-items-center justify-content-center m-0 p-0 buttongroup-container">
              <div className="add-btn btn btn-sm btn-danger m-0" onClick={(e) => {e.stopPropagation(); increaseCartQuantity(id);}}>+</div>
              <div className="d-flex align-items-baseline item-quantity ">
                <span><h3>{quantity}</h3></span> in Cart
              </div>
              <div className="btn btn-sm btn-danger subt-btn m-0" onClick={(e) => {e.stopPropagation(); decreaseCartQuantity(id);}}>-</div>
              <div className="btn btn-sm btn-danger remove-btn m-0" onClick={(e) => {e.stopPropagation(); removeFromCart(id);}}>Remove</div>
            </div>
          </>
        )}
    </>
